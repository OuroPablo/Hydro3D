	...
	integer :: cpu_next,cpu_prev
     ...
     & ycor(ndo,2),zcor(ndo,1),zcor(ndo,2)
        ...
        rdiv=1
          ...
          dx_nx = lx_loc/REAL(idom)
          dy_ny = ly_loc/REAL(jdom)
          dz_nz = lz_loc/REAL(kdom)
          ndo=0
        do k=1,kdom
          do j=1,jdom
            do i=1,idom
              ndo=ndo+1
             xcor(ndo,1)=dx_nx*(i-1) ;  xcor(ndo,2)=dx_nx*i
             ycor(ndo,1)=dy_ny*(j-1) ;  ycor(ndo,2)=dy_ny*j
             zcor(ndo,1)=dz_nz*(k-1) ;  zcor(ndo,1)=dz_nz*k
        enddo;enddo;enddo
