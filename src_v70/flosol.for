...
!Generate inlet turbulent field with Synthetic Eddy Method
          IF (bc_w.eq.8 .and. myrank.eq.0) call SEM  
...
!Write outflow planes
	  if (save_inflow .and. itime.ge.itime_start) call write_inflow !set itime since when you'd like to write inflow planes
...
!Lagrangian Particle Tracking final checks
        ...
        ENDIF
        if (LPT) then									
	   if((mod(itime,tsteps_pt).eq.0).and.(itime.gt.itime_start)) then 			
